    ...
    <main className="mx-auto max-w-6xl px-4 py-10 md:px-6">
      ...
      <section className="mb-10">
        ...
        <div className="rounded-xl border border-neutral-700 bg-neutral-900/80 p-4">
          ...
          {/* 바 트랙 */}
          <div className="relative flex h-12 w-full rounded-lg bg-neutral-800">
            {SLOT_LABELS.map((_, i) => (
              <div
                key={i}
                className="h-full flex-1 border-r border-neutral-700 last:border-r-0"
                title={slotToTime(i)}
              />
            ))}
            {/* 예약된 구간 오버레이 */}
            {bookingsForDay.map((b) => {
              ...
              return (
                <div
                  key={b.id}
                  className="absolute top-1 h-10 rounded bg-amber-500/80 py-1 px-2 text-xs font-medium text-black"
                  style={{
                    left: `${left}%`,
                    width: `${width}%`,
                    minWidth: "60px",
          ...
          <p className="mt-2 text-xs text-neutral-500">
            위 바는 해당 날짜의 예약된 시간대를 표시합니다. 노란색: 예약 구간, 대표자 이름 표시.
          </p>
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <form
          ...
          className="flex flex-col gap-4 rounded-xl border border-neutral-700 bg-neutral-900/80 p-4"
        >
          <div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-neutral-600 bg-neutral-800 px-3 py-2 text-white placeholder-neutral-500"
              required
            />
          </div>
          <button
            type="submit"
            className="rounded-lg bg-amber-500 px-4 py-3 text-sm font-medium text-black hover:bg-amber-400"
          >
            예약하기
          </button>
        </form>
      ...
      <section>
        ...
        {myBookings.length === 0 ? (
          <p className="rounded-xl border border-neutral-700 bg-neutral-900/80 p-4 text-sm text-neutral-400">
          ...
          <ul className="space-y-2">
            {myBookings.map((b) => (
              <li
                key={b.id}
                className="flex flex-wrap items-center gap-2 rounded-xl border border-neutral-700 bg-neutral-900/80 px-4 py-3 text-sm"
              >
                <span className="text-neutral-400">{b.date}</span>
